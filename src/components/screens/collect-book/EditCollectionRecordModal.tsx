...
export default function EditCollectionRecordModal({
    ...
    const content = (
        <View className={`${isScreen ? "flex-1 bg-background" : "flex-1 bg-black/60 justify-center items-center px-4"}`}>
            <View className={`w-full max-w-md relative overflow-hidden ${isScreen ? "flex-1 max-w-none pt-12" : ""}`}>
                ...
                <Card className={`w-full bg-card border border-border/40 shadow-2xl shadow-black/40 ${isScreen ? "flex-1 border-0 shadow-none rounded-none" : "rounded-[32px]"}`}>
                    ...
                    {/* Action Buttons */}
                    <View className={`flex-row gap-4 p-8 pt-4 ${isScreen ? "pb-12" : ""}`}>
                        <Pressable
                            onPress={onClose}
